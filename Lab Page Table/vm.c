...
freewalk(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
     // 根据flag标识位的设置来判断当前页表的层数，只有在页表的最后一级，才可进行读、写、执行，如果不是最低一层，则继续往下走，直到最后一层开始回溯。
...
void 
pagePrint(pagetable_t pagetable, uint64 level)
{
  for(int i = 0; i < 512; i++)
  {
    pte_t pte = pagetable[i];
    if(pte & PTE_V)
    {
      ...
      {
        pagePrint((pagetable_t)child, level+1);
      }    
    }
  }
}
